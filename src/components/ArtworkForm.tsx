        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            {imageUrl && (
              <img
                src={imageUrl}
                alt="Aperçu"
                className="w-full h-40 object-cover rounded-md mb-2 border border-bordure"
              />
            )}
            <label className="flex items-center gap-2 px-3.5 py-2.5 rounded-md border border-dashed border-bordure text-sm cursor-pointer text-encre/60">
              <UploadCloud size={16} />
              {uploadingImage ? "Envoi en cours..." : imageUrl ? "Changer l'image" : "Choisir une image"}
          ...
          <div>
            <label className="block text-sm mb-1.5 text-encre/80">
              Fichier 3D / RA (.glb) — optionnel
            </label>

            {model3dUrl && (
              <div className="mb-2 rounded-md overflow-hidden border border-bordure bg-platre">
                {/* @ts-ignore - web component Google model-viewer */}
                <model-viewer
                  src={model3dUrl}
                  camera-controls
                  auto-rotate
                  style={{ width: "100%", height: "220px", backgroundColor: "#F7F2EA" }}
                />
                <p className="text-xs text-center py-1.5 text-encre/50">
            ...
            <label className="flex items-center gap-2 px-3.5 py-2.5 rounded-md border border-dashed border-bordure text-sm cursor-pointer text-encre/60">
              <UploadCloud size={16} />
              {uploading3d ? "Envoi en cours..." : model3dUrl ? "Changer le fichier 3D" : "Choisir un fichier .glb"}
